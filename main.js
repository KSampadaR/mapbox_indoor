...
roomPoints.features = roomPoints.features.filter(
	(feature) => feature.properties.use != "Corridor Point"
);
roomPoints.features = roomPoints.features.filter(
...
function floorToggler(value) {
	...
	pointsClone.features = pointsClone.features.filter(
		(feature) => feature.properties.level == value
	);
	...
	roomPoints.features = roomPoints.features.filter(
		(feature) => feature.properties.Name != "Corridor Point"
	);

	// obstacle
	let obstacle = polygon.features.filter(
		(feature) => feature.properties.level == value
	);
...
function searchAddress(value) {
	let data = activeTab
		? JSON.parse(JSON.stringify(roomPoints))
		: JSON.parse(JSON.stringify(points));
	console.log(data);

	data.features = data.features.filter((feature) => {
		if (feature.properties.Name.toLowerCase().includes(value.toLowerCase())) {
			return feature;
		}
	});

	data.features =
		data.features.length > 6 ? data.features.slice(0, 6) : data.features;
...
function updateSearchResultContainer(features) {
	...
	features.forEach((feature) => {
		...
		listGroupItem.innerHTML +=
			"<div class='text-small'>, Floor " + feature.properties.level + "</div>";
...
function handleClickEvent(value, obj) {
	...
	if (activeTab == "") {
		...
		let feature = [...points.features].find(
			(feature) => feature.properties.fid == featureId
		);
		...
		startMarker.setLngLat(coordinates).addTo(map);
...
// function
let obstacle = obstacles.features.filter(
	(feature) => feature.properties.level == 0
);
	...
	routePromise
		.then((data) => {
			if (data.type) {
				...
				summaryInfo.innerHTML = "";
				summaryInfo.innerHTML =
					"<div><h5><span class='text-success'>" +
					time +
					"   </span>(" +
					Math.ceil(distance) +
					" m)</h5><p>Via Main Corridor</p></div>";
				summaryInfo.innerHTML +=
					'<button class="btn" id="preview-steps">PREVIEW</button>';
...
function getDirections(data) {
	...
	for (let i = 0; i < directionCoords.length; i++) {
		...
		if (i == coordsLength) {
			...
			direction.from = getPointName(directionCoords[i][2]);
			direction.distance = getDistance(
				directionCoords[i],
				directionCoords[i + 1]
			);
			direction.bearing = getBearing(
				directionCoords[i],
				directionCoords[i + 1]
			);
		}
...
function getPointName(fid) {
	// console.log(fid)
	return points.features.find((feature) => feature.properties.fid == fid)
		.properties.Name;
}
...
function updateDirectionsTab(directions) {
	...
	directions.forEach((direction, i) => {
		...
		if (i == 0) {
			...
			listItem.innerHTML +=
				"<div class='list-direction'>" + direction.from + "</div>";
			listItem.innerHTML +=
				"<div class='directions-step-distance'>" + distance + " m</div>";
